...
class actions_dashboard {
	function handle(&$params){
		...
		if(1){
			...
			$app = df_get_records_array('accounts_payable', array('post_status'=>'Pending'));
			$css = df_get_records_array('call_slips', array('status'=>'RDY'));
			df_display(array("call_slips"=>(count($css)),"payable_pending"=>(count($app)),"journal_pending"=>(count($jpp)),"ipo_pending"=>(count($ipo)),"spo_pending"=>(count($spo)),"opo_pending"=>(count($opo)),"rpo_pending"=>(count($rpo))), 'dashboard.html');
		}
		if(0){
