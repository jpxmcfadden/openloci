		...
		switch($action_section){
			case NULL: { //Initial State: Select Period based on Period Type
				...
				foreach($records as $record){
					...
					if($period_type == "month"){
						...
						$date_rng = ">=" . date("Y-m-1",strtotime($record['ledger_date']['year'] . '-' . $record['ledger_date']['month'])) . " AND <=" . date("Y-m-t",strtotime($record['ledger_date']['year'] . '-' . $record['ledger_date']['month']));
					}
					elseif($period_type == "year"){
						$date_str = $record['ledger_date']['year'];
						$date_rng = ">=" . $record['ledger_date']['year'] . "-01-01 AND <=" . $record['ledger_date']['year'] . "-12-31";
					}
					elseif($period_type == "quarter"){
						//$quarter = ceil(date('m',strtotime($record['ledger_date']['year'] . "-" . $record['ledger_date']['month']))/3);
						$quarter = ceil($record['ledger_date']['month'] / 3);
						$date_str = $record['ledger_date']['year'] . ' - Quarter #' . $quarter;
						$date_rng = ">=" . $record['ledger_date']['year'] . '-' . ((($quarter - 1)*3)+1) . "-01 AND <=" .  date("Y-m-t",strtotime($record['ledger_date']['year'] . '-' . ($quarter *3) . '-01'));
					}
			...
			case "display": { //Check for unposted batches & show, Show Trial Balance / Income Statement / Balance Sheets
				...
				foreach($records as $record){
					...
					echo "(".$account_data[$journal_entry->val('account_id')]['debit'].")(".$account_data[$journal_entry->val('account_id')]['credit'].")<BR>"; 
						...
						//echo $journal_entry->val('ledger_id') . ":" . $journal_entry->val('journal_id'). " :: ". $journal_entry->val('account_id') . " -- Debit = " . $journal_entry->val('debit') . ", Account Debit = " . $account_data[$journal_entry->val('account_id')]['debit'] . ", Total Debit = " . $total_debit . "<br>";
						//echo $journal_entry->val('ledger_id') . ":" . $journal_entry->val('journal_id') . " :: ". $journal_entry->val('account_id') . " -- Credit = " . $journal_entry->val('credit') . ", Account Credit = " . $account_data[$journal_entry->val('account_id')]['credit'] . ", Total Credit = " . $total_credit . "<br><br>";
					}
				}
				ksort($account_data); //use ksort() to sort account numbers numerically accending
