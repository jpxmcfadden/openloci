...
class actions_inventory_overide {
	
	function handle(&$params){

//Copied from xataface/actions/edit.php
//Modified to specifically show the "quantity" field for editing.
//All other fields have been hidden.
//Modifications and comment lines (to comment out parts of the script I don't want to run - eg. Tabs), are all located on the first line of each row, so that it's easy to see what was edited.
...
//Set *all* field widget types to "hidden"
foreach($currentRecord->_table->_fields as &$rec){
	$rec["widget"]["type"] = "hidden";
}

//Set "quantity" field widget type to "text" - so that it can be edited
$currentRecord->_table->_fields["quantity"]["widget"]["type"] = "text";
