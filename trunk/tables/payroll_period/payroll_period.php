		...
		foreach($payroll_data as $payroll_entry){
			...
			if(is_array($payroll_entry)){
				...
				$childString .= '<table><tr><td style="padding-left:15px; padding-right:15px; width: 600px;">';
					...
					$childString .= '<tr><th>Net Pay</th><td>$' . ($payroll_entry["gross_income"] - $payroll_entry["total_deductions"]) .
										'</td><th>YTD</th><td>$'. ($payroll_entry["gross_income_ytd"] - $payroll_entry["total_deductions_ytd"]) . '</td></tr>';
...
//$childString .= "<pre>".print_r($payroll_data,true)."</pre>";
	...
	function payroll_from_entries($record){
		...
		foreach($payroll_entries as $e_key=>$entry){
			...
			$payroll_data[$e_key]["gross_income"] = $entry->val("gross_income");
			$payroll_data[$e_key]["wages"] = $entry->val("wages");
			$payroll_data[$e_key]["ss_wages"] = $entry->val("ss_wages");
			$payroll_data[$e_key]["total_deductions"] = $entry->val("total_deductions");
			$payroll_data[$e_key]["total_contributions"] = $entry->val("total_conributions");
			$payroll_data[$e_key]["gross_income_ytd"] = $entry->val("gross_income_ytd");
			$payroll_data[$e_key]["wages_ytd"] = $entry->val("wages_ytd");
			$payroll_data[$e_key]["ss_wages_ytd"] = $entry->val("ss_wages_ytd");
			$payroll_data[$e_key]["total_deductions_ytd"] = $entry->val("total_deductions_ytd");
			...
			// ********** INCOME SECTION **********
				...
				foreach($addition_records as $a_key=>$addition_record){
					...
					//Total
					$payroll_data[$e_key]["income"]["income_entry_".$a_key]["total_amount"] = $addition_record->val('posted_amount');
					
					//YTD
					$payroll_data[$e_key]["income"]["income_entry_".$a_key]["ytd"] = $addition_record->val('posted_ytd');;
			...
			// **** DEDUCTIONS SECTION ****
				...
				foreach($addition_records as $a_key=>$addition_record){
					...
					//Amount - Base
					if($addition_record->val('amount_base') != null)
						$payroll_data[$e_key]["deductions"]["deduction_entry_".$a_key]["base"] = $addition_record->val('amount_base');
					else
					...
					//Total
					$payroll_data[$e_key]["deductions"]["deduction_entry_".$a_key]["total_amount"] = $addition_record->val('posted_amount');
					
					//YTD
					$payroll_data[$e_key]["deductions"]["deduction_entry_".$a_key]["ytd"] = $addition_record->val('posted_ytd');;
			...
			// **** CONTRIBUTIONS SECTION ****
				...
				foreach($addition_records as $a_key=>$addition_record){
					...
					//Total
					$payroll_data[$e_key]["contributions"]["contribution_entry_".$a_key]["total_amount"] = $addition_record->val('posted_amount');

					//YTD
					$payroll_data[$e_key]["contributions"]["contribution_entry_".$a_key]["ytd"] = $addition_record->val('posted_ytd');
	...
	function calculate_payroll($record){
		...
		//Initialize data
		$payroll_data = array();
		$payroll_data['period_gross'] = 0;
		$payroll_data['period_wages'] = 0;
		$payroll_data['period_ss_wages'] = 0;
		$payroll_data['period_deductions'] = 0;
		$payroll_data['period_contributions'] = 0;
		...
		foreach($payroll_entries as $e_key=>$entry){
			...
			//Add to Period Total
			$payroll_data["period_gross"] += $payroll_data[$e_key]["gross_income"];
			$payroll_data["period_wages"] += $payroll_data[$e_key]["wages"];
			$payroll_data["period_ss_wages"] += $payroll_data[$e_key]["ss_wages"];
			$payroll_data["period_deductions"] += $payroll_data[$e_key]["total_deductions"];
			$payroll_data["period_contributions"] += $payroll_data[$e_key]["total_contributions"];
	...
	function post_payroll($record){
		...
		*/
			...
			//Set "Payroll Period" status / posted date / period totals.
			$record->setValues(array(
								'status'=>"Posted",
								'post_date'=>date("Y-m-d"),
								'period_gross'=>$payroll_data["period_gross"],
								'period_wages'=>$payroll_data["period_wages"],
								'period_ss_wages'=>$payroll_data["period_ss_wages"],
								'period_deductions'=>$payroll_data["period_deductions"],
								'period_contributions'=>$payroll_data["period_contributions"],
								'year'=>$payroll_data["year"]
			));
			...
			if($save_error == 0){
					...
					if(is_array($payroll_entry)){
						...
						foreach($addition_types as $addition_type){
							...
							foreach($payroll_entry[$addition_type] as $addition_entry){
								...
								$addition_record->setValue('posted_ytd', $ytd_amount);
								$addition_record_ytd->setValue('ytd_amount', $ytd_amount);
				...
				$glj_record->setValues(array(
					...
					'credit'=>$payroll_data['period_gross']-$payroll_data['period_deductions']
				));
