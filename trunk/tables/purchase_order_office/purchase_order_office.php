...
class tables_purchase_order_office {
	...
	function getPermissions(&$record){
		...
		if( isUser() ){
			$userperms = get_userPerms('purchase_order_office');
			if($userperms == "view")
	...
	function getTitle(&$record){
		return "Office Purchase Order #" . $record->strval('purchase_id');
	}

	function purchase_id__display(&$record){
		return "O".$record->val('purchase_id');
	}
	...
	function section__pricing(&$record){
		...
		$childString = "";
			...
			$purchaseorderRecords = $record->getRelatedRecords('purchase_order_office_items');
			$total_all_items = 0;
	...
	function afterInsert(&$record){
		...
		$record->setValue('purchase_order_id', "O".$record->val('purchase_id'));
		$record->save();
	}	
